...
interface JoinTokenDialogProps {
  roomId: string;
  joinToken: string;
  onClose: () => void;
}

const JoinTokenDialog: React.FC<JoinTokenDialogProps> = ({ roomId, joinToken, onClose }) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
      <div className="bg-white rounded-lg shadow-lg p-6 max-w-md mx-4 w-full">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold">ルーム招待情報</h2>
          <button
            onClick={onClose}
        ...
        <div className="mb-6">
          <p className="text-gray-600 mb-2">
            以下の招待URLまたは参加トークンを共有して、他のユーザーをこのルームに招待できます。
          </p>
        </div>
